...
class inducer {
 public:
  inducer(environment env = environment())
      : env_comm_(env.communicator()),
        env_rank_(env.rank()),
        env_size_(env.size()),
  ...
  inline size_t alltoallv_compute_target(DataType* send_data,
    ...
    if constexpr (left_to_right) {
      MPI_Alltoallv(send_data, sc_buffer_.data(), sd_buffer_.data(),
                    mpi_datatype_, cur_target_pos, rc_buffer_.data(),
                    rd_buffer_.data(), mpi_datatype_, env_comm_);
    } else /*if (!left_to_right)*/ {
      MPI_Alltoallv(send_data, sc_buffer_.data(), sd_buffer_.data(),
                    mpi_datatype_, cur_target_pos - recv_elements,
                    rc_buffer_.data(), rd_buffer_.data(), mpi_datatype_,
                    env_comm_);
    }
    return recv_elements;
  }

  void induce_right_to_left(
      std::vector<DataType*>& send_data, std::vector<size_t>& local_counts,
    ...
    MPI_Allreduce(local_containing.data(), preceding_sizes.data(),
                  considered_chars, MPI_UNSIGNED_LONG_LONG, MPI_SUM, env_comm_);

    std::vector<size_t> prec_send_buffer(env_size_ * considered_chars, 0);
    MPI_Allgather(local_counts.data(), considered_chars, MPI_UNSIGNED_LONG_LONG,
                  prec_send_buffer.data(), considered_chars,
                  MPI_UNSIGNED_LONG_LONG, env_comm_);
    ...
    MPI_Alltoall(send_count_buffer.data(), considered_chars, MPI_INT,
                 receive_count_buffer.data(), considered_chars, MPI_INT,
                 env_comm_);
    ...
    for (size_t i = 0; i < considered_chars; ++i) {
      ...
      size_t const res =
          alltoallv_compute_target<false>(send_data[i], cur_target_pos[i]);
      tar_buckets[i]->containing += res;
    }
  }

  void induce_left_to_right(
      std::vector<DataType*>& send_data, std::vector<size_t>& local_counts,
      std::vector<size_t>& local_containing,
      std::vector<size_t>& global_bucket_sizes,
      std::vector<DataType*>& cur_target_pos,
    ...
    for (size_t i = 0; i < considered_chars; ++i) {
      ...
      for (int32_t cur_rank = target_rank(preceding_sizes[i], slice_size);
        ...
        const size_t to_send = std::min(
            (cur_rank + 1) * slice_size + missing_slice - preceding_sizes[i],
            local_counts[i]);
        send_count_buffer[i + (considered_chars * cur_rank)] = to_send;
        local_counts[i] -= to_send;
        preceding_sizes[i] += to_send;
    ...
    for (size_t i = 0; i < considered_chars; ++i) {
      ...
      size_t const res =
          alltoallv_compute_target<true>(send_data[i], cur_target_pos[i]);
      tar_buckets[i]->containing += res;
    }
  }
