...
inline std::vector<DataType> gather(DataType& send_data, int32_t target,
  ...
  MPI_Gather(&send_data, 1, dtm.get_mpi_type(), result.data(), 1,
             dtm.get_mpi_type(), target, env.communicator());
  return result;
}

template <typename DataType>
inline void gatherv_small(DataType* send_data, int32_t send_count,
                          int32_t target, DataType* recv_buffer,
                          environment env = environment()) {
  ...
  data_type_mapper<DataType> dtm;
  MPI_Gatherv(send_data, send_count, dtm.get_mpi_type(), recv_buffer,
              receiving_sizes.data(), receiving_offsets.data(),
              dtm.get_mpi_type(), target, env.communicator());
}

template <typename DataType>
inline void gatherv(DataType* send_data, size_t send_count, int32_t target,
                    DataType* recv_buffer, environment env = environment()) {
  ...
  if (global_to_big) {
    std::vector<MPI_Request> mpi_requests(env.rank() == target ? env.size() + 1
                                                               : 1);

    if (env.rank() == target) {
      for (std::int32_t i = 0; i < env.size(); ++i) {
        auto receive_type = get_big_type<DataType>(receiving_sizes[i]);
        MPI_Irecv(recv_buffer + receiving_offsets[i], 1, receive_type, i, 44227,
                  env.communicator(), &mpi_requests[1 + i]);
      }
    }
    auto send_type = get_big_type<DataType>(send_count);
    MPI_Isend(send_data, 1, send_type, target, 44227, env.communicator(),
              &mpi_requests[0]);
