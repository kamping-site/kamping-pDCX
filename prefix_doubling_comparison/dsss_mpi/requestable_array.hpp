...
class requestable_array {
 public:
  requestable_array(std::vector<DataType>& data, size_t total_size,
      ...
      : local_size_(data.size()),
        slice_size_(total_size / env.size()),
        data_(data.data()),
        env_(env) {
    MPI_Win_create(data_, local_size_, sizeof(DataType), MPI_INFO_NULL,
                   env_.communicator(), &win_);
  }
  ...
  inline DataType& operator[](size_t index) { return data_[index]; }

  inline DataType operator[](size_t index) const { return data_[index]; }

  inline DataType& back() { return data_[local_size_ - 1]; }

  inline DataType back() const { return data_[local_size_ - 1]; }
