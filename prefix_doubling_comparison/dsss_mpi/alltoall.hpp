...
inline std::vector<DataType> alltoall(std::vector<DataType>& send_data,
  ...
  MPI_Alltoall(send_data.data(), send_data.size() / env.size(),
               dtm.get_mpi_type(), receive_data.data(),
               send_data.size() / env.size(), dtm.get_mpi_type(),
               env.communicator());
  return receive_data;
}
...
inline std::vector<DataType> alltoallv_small(
  ...
  std::vector<DataType> receive_data(receive_counts.back() +
                                     receive_displacements.back());
  ...
  data_type_mapper<DataType> dtm;
  MPI_Alltoallv(send_data.data(), send_counts.data(), send_displacements.data(),
                dtm.get_mpi_type(), receive_data.data(), receive_counts.data(),
                receive_displacements.data(), dtm.get_mpi_type(),
                env.communicator());
  return receive_data;
}
...
inline std::vector<DataType> alltoallv(std::vector<DataType>& send_data,
  ...
  size_t local_receive_count =
      std::accumulate(receive_counts.begin(), receive_counts.end(), 0);
  ...
  if (global_max < env.mpi_max_int()) {
    ...
    for (int32_t i = 0; i < env.size(); ++i) {
      ...
      MPI_Isend(send_data.data() + send_displacements[target], 1, send_type,
...
inline dsss::indexed_string_set<IndexType> alltoallv_indexed_strings(
  ...
  std::vector<dsss::char_type> receive_data =
      alltoallv(real_send_data, send_counts_char, env);
  std::vector<IndexType> receive_data_indices =
      alltoallv(index_send_data, send_counts_strings, env);
