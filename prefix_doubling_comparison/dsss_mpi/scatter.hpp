...
inline DataType scatter(std::vector<DataType>& send_data, int32_t root,
  ...
  DataType result;
  MPI_Scatter(send_data.data(), 1, dtm.get_mpi_type(), &result, 1,
...
static inline size_t scatterv_small(DataType* send_data,
                                    ...
                                    int32_t root, DataType* recv_buffer,
                                    environment env = environment()) {
  ...
  MPI_Scatterv(send_data, send_counts.data(), offsets.data(),
               dtm.get_mpi_type(), recv_buffer, recv_count, dtm.get_mpi_type(),
               root, env.communicator());
  return recv_count;
}

template <typename DataType>
static inline size_t scatterv(DataType* send_data, size_t data_size,
                              std::vector<size_t>& send_counts, int32_t root,
  ...
  if (data_size < env.mpi_max_int()) {
    ...
    if (env.rank() == root) {
      for (int32_t i = 0; i < env.size(); ++i) {
        ...
        MPI_Isend(send_data + offsets[i], 1, send_type, i, 44227,
                  env.communicator(), &mpi_requests[i]);
      }
    }
    auto recv_type = get_big_type<DataType>(recv_count);
    MPI_Irecv(recv_buffer, 1, recv_type, root, 44227, env.communicator(),
...
template <typename DataType>
static inline size_t scatter_distibuted(DataType* send_data, size_t data_size,
                                        int32_t root, DataType* recv_buffer,
                                        environment env = environment()) {
