...
DataType ex_prefix_sum(DataType local_data,
  ...
  DataType result;
  MPI_Exscan(&local_data, &result, type_mapper<DataType>::factor(),
...
template <typename DataType>
DataType prefix_sum(DataType local_data,
                    environment const& env = environment()) {
  static_assert(std::numeric_limits<DataType>::is_integer,
                "Only integers are allowed for scan.");

  DataType result;
  MPI_Scan(&local_data, &result, type_mapper<DataType>::factor(),
           type_mapper<DataType>::type(), MPI_SUM, env.communicator());
  return result;
}

template <typename DataType>
DataType rev_prefix_sum(DataType local_data,
                        environment const& env = environment()) {
  static_assert(std::numeric_limits<DataType>::is_integer,
                "Only integers are allowed for ex_prefix_sum.");
